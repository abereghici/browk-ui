...
import {CustomizationProvider} from '@browk-ui/customization';
import {Box} from '../src';
...
describe('Backgrounds', () => {
  it('should render single values', (): void => {
    render(
      <Theme.Provider theme="default">
        <Box backgroundColor="colorBackground" data-testid="box">
          background single
        </Box>
      </Theme.Provider>
    );
  ...
  it('should render responsive values', () => {
    render(
      <Theme.Provider theme="default">
        <Box backgroundColor={['colorBackground', 'colorBackgroundStrong']} data-testid="box">
    ...
    const renderedBox = screen.getByTestId('box');
    expect(renderedBox).toHaveStyleRule('background-color', 'rgb(255, 255, 255)');
    expect(renderedBox).toHaveStyleRule('background-color', 'rgb(250, 250, 250)', {
...
describe('Color mappings', () => {
  it('should map single color values', (): void => {
    ...
    const renderedBox = screen.getByTestId('box');
    expect(renderedBox).toHaveStyleRule('background-color', 'rgb(255, 255, 255)');
    expect(renderedBox).toHaveStyleRule('border-color', 'rgb(241, 241, 241)');
    expect(renderedBox).toHaveStyleRule('border-bottom-color', 'rgb(241, 241, 241)');
    expect(renderedBox).toHaveStyleRule('border-left-color', 'rgb(241, 241, 241)');
    expect(renderedBox).toHaveStyleRule('border-right-color', 'rgb(241, 241, 241)');
    expect(renderedBox).toHaveStyleRule('border-top-color', 'rgb(241, 241, 241)');
    expect(renderedBox).toHaveStyleRule('color', 'rgb(0, 0, 0)');
  });
  ...
  it('should map pseudo selector color values', () => {
    ...
    const renderedBox = screen.getByTestId('box');
    expect(renderedBox).toHaveStyleRule('background-color', 'rgb(255, 255, 255)', {target: ':hover'});
    expect(renderedBox).toHaveStyleRule('border-color', 'rgb(241, 241, 241)', {target: ':hover'});
    expect(renderedBox).toHaveStyleRule('color', 'rgb(0, 0, 0)', {target: ':hover'});
  });
  ...
  it('should map responsive pseudo selector color values', () => {
    render(
      <Theme.Provider theme="default">
        <Box
          _hover={{
            backgroundColor: ['colorBackground', 'colorBackgroundStrong'],
          }}
          data-testid="box"
        >
          background responsive
...
describe('Shadows', () => {
  it('should render single values', (): void => {
    ...
    const renderedBox = screen.getByTestId('box');
    expect(renderedBox).toHaveStyleRule('box-shadow', '0 4px 4px rgba(51,51,51, 0.04),0 4px 16px rgba(51,51,51, 0.08)');
  });

  it('should render responsive values', () => {
    render(
      <Theme.Provider theme="default">
        <Box boxShadow={['shadow', 'shadowHover']} data-testid="box">
          shadow responsive
        </Box>
      </Theme.Provider>
    );
    ...
    expect(renderedBox).toHaveStyleRule(
      'box-shadow',
      '0 4px 4px rgba(51,51,51, 0.04),0 4px 24px rgba(51,51,51, 0.24)',
      {
        media: 'screen and (min-width:25rem)',
      }
    );
...
describe('HTML attributes', () => {
  it('should set a default element data attribute', () => {
    render(<Box>box</Box>);
    expect(screen.getByText('box').getAttribute('data-browk-ui-element')).toEqual('BOX');
  });
  it('should set a custom element data attribute', () => {
    render(<Box element="foo">box</Box>);
    expect(screen.getByText('box').getAttribute('data-browk-ui-element')).toEqual('foo');
  });
});
...
describe('Customization', () => {
  it('should add custom styles to a component provided as element styles on the customization provider', (): void => {
    render(
      <CustomizationProvider
        baseTheme="default"
        elements={{CUSTOM_BOX: {color: 'colorText', textDecoration: 'underline'}}}
      >
  ...
  it('should add custom styles to pseudo selectors when provided as element styles on the customization provider', (): void => {
    render(
      <CustomizationProvider baseTheme="default" elements={{CUSTOM_BOX: {':hover': {color: 'colorTextInverse'}}}}>
        <CustomizableBoxExample element="CUSTOM_BOX" data-testid="customizable-box">
          Custom Box
        </CustomizableBoxExample>
      </CustomizationProvider>
  ...
  it('should override existing variant styles when provided as element styles on the customization provider', (): void => {
    render(
      <CustomizationProvider
        baseTheme="default"
        elements={{CUSTOM_TEXT: {variants: {primary: {backgroundColor: 'colorBackground'}}}}}
      >
        <CustomizableBoxExample element="CUSTOM_TEXT" variant="primary" data-testid="customizable-box">
          Custom Box
        </CustomizableBoxExample>
