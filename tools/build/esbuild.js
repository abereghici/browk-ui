...
/**
 * ESBuild plugin to fix CJS builds in Browk UI
 ...
 */
const BrowkUICJSResolverPlugin = {
  name: 'BrowkUICJSResolver',
  setup(build) {
...
/**
 * ESBuild handles externals literally so that `@browk-ui/design-tokens` won't
 * get bundled, but `@browk-ui/design-tokens/dist/theme/sengrid/tokens` will.
 * This function adds a wildcard lookup for all externals.
 *
 * Note: adding `/*` breaks external-ing the normal `@browk-ui/design-tokens` because
...
function build(packageJson) {
  ...
  esbuild
    .build({
      ...
      plugins: [BrowkUICJSResolverPlugin],
    })
